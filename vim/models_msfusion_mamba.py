...
from TS_Mixer_Mamba import Temporal_Mixer_3MSTCT
from rope import *
import random
...
class HierarchicalVisionMamba(nn.Module):
    def __init__(self, 
        ...
        for i in range(len(depths)):
            ...
            if i == 0:
                self.blocks.append(TemporalConvBlock(embed_dims[0], embed_dims[i], kernel_size=3, stride=1))
            else:
                self.blocks.append(TemporalConvBlock(embed_dims[i-1], embed_dims[i], kernel_size=3, stride=2))

            self.blocks.append(self._create_mamba_block(embed_dims[i], d_state, depths[i], **kwargs))
        
        # self.temporal_mixer = Temporal_Mixer(inter_channels=embed_dims, embedding_dim=512) 
        self.temporal_mixer = Temporal_Mixer_3MSTCT(inter_channels=embed_dims, embedding_dim=embed_dims[1]) 

        # self.temporal_downsample = nn.Conv1d(1792, 1024, kernel_size=3, stride=1, padding=1) # CLIP(Charades)
        # self.temporal_downsample = nn.Conv1d(448, 256, kernel_size=3, stride=1, padding=1) # I3D(Charades)

        # self.temporal_downsample = nn.Conv1d(4375, 2000, kernel_size=3, stride=1, padding=1) # CLIP(Smarthome)
        # self.temporal_downsample = nn.Conv1d(3500, 2000, kernel_size=3, stride=1, padding=1) # CLIP(Smarthome)

        # self.temporal_downsample = nn.Conv1d(3500, 2000, kernel_size=3, stride=1, padding=1) # CLIP(Smarthome)

        # self.temporal_interaction = self._create_mamba_block(embed_dim=embed_dims[1], d_state=d_state, depth=1, **kwargs) ## B,T,C
    ...
    def _create_mamba_block(self, embed_dim, d_state, depth, **kwargs):
        return VisionMamba(
            # img_size=1,  # Not used in forward pass
            # patch_size=1,  # Not used in forward pass
            # in_chans=1,  # Not used in forward pass
            # num_classes=0,  # We don't need the classifier here
            embed_dim=embed_dim,
            depth=depth,
            d_state=d_state,
            rms_norm=True, 
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        # x = x.permute(0, 2, 1)
        # x = resize(x, size=256, mode='linear', align_corners=False)
        # breakpoint()
...
@register_model
def hierarchical_vim(pretrained=False, **kwargs):
    model = HierarchicalVisionMamba(
        # patch_size=16,
        # embed_dims=[256, 384, 576, 864],
        # temporal_dims=[256, 128, 64, 32],
        # depths=[1, 1, 1, 1],
        ...
        temporal_dims=[256, 128, 64],
        # temporal_dims=[512, 256, 128],
        depths=[1,1,1],
        # embed_dims=[256, 384],
        # temporal_dims=[256, 128],
        # depths=[1, 1],
        d_state=16,
        rms_norm=True,
        residual_in_fp32=True,
...
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='all', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=False, if_divide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=384, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='all', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=False, if_divide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
@register_model 
def vim_small_patch16_224_bimambav2_depth36_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=384, depth=36, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='all', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=False, if_divide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
def vim_base_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_middle_cls_token_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=768, d_state=16, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='all', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=False, if_divide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
